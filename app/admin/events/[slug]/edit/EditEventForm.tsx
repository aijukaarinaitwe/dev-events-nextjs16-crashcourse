...
export default function EditEventForm({ event }: EditEventFormProps) {
    ...
    // Pre-fill states with current database values
    const [title, setTitle] = useState(event.title || '');
    // Format date string to YYYY-MM-DD for native HTML date input
    ...
    const [agenda, setAgenda] = useState(event.agenda ? event.agenda.join('\n') : '');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (imageFile) {
                ...
                formDataPayload.append('image', imagePreview);
            }
            ...
            if (res.ok) {
                ...
                setError(data.message || data.error || 'Failed to update event. Please check inputs.');
            }
        } catch (err) {
    ...
    return (
        <div className="bg-dark-100 border border-border-dark card-shadow rounded-[10px] p-6 md:p-8 space-y-6">
            <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                {success && (
                    <div className="p-4 bg-emerald-950/30 border border-emerald-500/20 text-emerald-400 rounded-md text-sm font-semibold text-center">
                ...
                >
                    {loading ? 'Updating Event Details...' : 'Save Changes'}
                </button>
            </form>
